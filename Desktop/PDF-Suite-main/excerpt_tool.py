...
# region agent log
def _agent_log(*, hypothesisId: str, location: str, message: str, data: dict, runId: str = "pre-fix") -> None:
    try:
        log_path = str((Path(__file__).resolve().parent / "debug-68b904.log"))
...
# region agent log
def _require_fitz(*, location: str, runId: str = "pre-fix"):
    """Return pymupdf module or raise an actionable error.

    Also logs whether fitz was available at runtime.
    """
    _agent_log(
        hypothesisId="A",
        location=location,
        message="require_fitz",
        data={"fitz_is_none": fitz is None},
        runId=runId,
    )
    if fitz is None:
...
# ---------------------------------------------------------------------------
# Wheel-to-horizontal-scroll event filter (same pattern as split_tool)
# ---------------------------------------------------------------------------
...
class ExcerptTool(QWidget):
    ...
    def _rebuild_pdf_list(self):
        ...
        while self._pdf_list_layout.count() > 1:
            ...
            if item is None:
                ...
                _agent_log(
                    hypothesisId="A",
                    location="excerpt_tool.py:_rebuild_pdf_list",
                    message="takeAt returned None unexpectedly",
                    data={"layoutCount": self._pdf_list_layout.count()},
                )
                # endregion agent log
                break
            w = item.widget()
            ...
            _agent_log(
                hypothesisId="A",
                location="excerpt_tool.py:_rebuild_pdf_list",
                message="takeAt returned item; inspected widget()",
                data={
                    "layoutItemType": type(item).__name__,
                    "hasWidget": w is not None,
                    "widgetType": type(w).__name__ if w is not None else None,
                    "layoutCount": self._pdf_list_layout.count(),
                },
            )
            # endregion agent log
            if w is not None:
        ...
        if hasattr(self, '_active_file_lbl'):
    ...
    def _render_page(self):
        ...
        _agent_log(
            hypothesisId="B",
            location="excerpt_tool.py:_render_page",
            message="render_page_scale",
            data={"cw": cw, "ch": ch, "pw": float(pw), "scale": float(scale), "page_index": self._current_page},
            runId="pre-fix",
        )
    ...
    def _render_thumb_batch(self, batch: int = 8):
        ...
        if doc is None:
            return
        fitz_mod = _require_fitz(location="excerpt_tool.py:_render_thumb_batch", runId="pre-fix")
        ...
        for i in range(start, end):
            ...
            try:
                ...
                pix   = page.get_pixmap(
                    matrix=fitz_mod.Matrix(scale, scale), alpha=False)
    ...
    def _canvas_to_pdf_rect(self, x0, y0, x1, y1):
        ...
        fitz_mod = _require_fitz(location="excerpt_tool.py:_canvas_to_pdf_rect", runId="pre-fix")
        p0 = fitz_mod.Point(x0 - self._page_ox,
                         y0 - self._page_oy) * self._inv_mat
        p1 = fitz_mod.Point(x1 - self._page_ox,
                         y1 - self._page_oy) * self._inv_mat
        r = fitz_mod.Rect(p0, p1)
        r.normalize()
        return r
    ...
    def _make_snippet_thumbnail(self, snip: Snippet) -> QPixmap | None:
        ...
        try:
            fitz_mod = _require_fitz(location="excerpt_tool.py:_make_snippet_thumbnail", runId="pre-fix")
            src_doc  = fitz_mod.open(snip.source_path)
            clip     = snip.crop_rect
    ...
    def _append_to_output(self, snip: Snippet):
        ...
        try:
            fitz_mod = _require_fitz(location="excerpt_tool.py:_append_to_output", runId="pre-fix")
            src_doc  = fitz_mod.open(snip.source_path)
            clip     = snip.crop_rect
    ...
    def _rebuild_output_doc(self):
        ...
        try:
            ...
            pass
        fitz_mod = _require_fitz(location="excerpt_tool.py:_rebuild_output_doc", runId="pre-fix")
        self._out_doc = fitz_mod.open()
