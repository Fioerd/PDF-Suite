...
def _fitz_pix_to_qpixmap(pix):
    """Convert a fitz Pixmap to a QPixmap."""
    try:
        data = pix.samples_mv
...
class PDFCanvas(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if vt._selected_words and fitz:
            # Expand each char rect by 0.5px left/right so adjacent chars
            # overlap. QPainterPath.simplified() then merges all intersecting
            # sub-rects into continuous line bands before a single fillPath
            # call — one paint per pixel, no dark edges, no per-letter boxes.
            sel_path = QPainterPath()
            for w in vt._selected_words:
                cx0, cy0, cx1, cy1 = vt._pdf_rect_to_canvas(
                    fitz.Rect(w[0], w[1], w[2], w[3]))
                sel_path.addRect(QRectF(cx0 - 0.5, cy0, (cx1 - cx0) + 1.0, cy1 - cy0))
    ...
    def _build_ui(self):
        ...
        if self._back_callback:
            ...
            back_btn.setStyleSheet(
                ...
                f"border-radius: 6px; font: 14px 'Segoe UI'; padding: 0 8px; }}"
                f"QPushButton:hover {{ background: {G100}; color: {G700}; }}"
            )
